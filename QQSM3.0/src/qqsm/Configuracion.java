...
public class Configuracion {
	...
	//Creamos los atributos que necesitaremos para jugar por consola y por ventanas
	...
	/*public static  void LeerFicheroPreguntasF() {
		...
		FileInputStream fis;
		try {
			fis = new FileInputStream("preguntasFaciles.dat");
			ObjectInputStream ois = new ObjectInputStream(fis);
			Pregunta a = (Pregunta) ois.readObject();
			int numPregunta = 1;

			while (a != null) {
				facil.put(new Integer(numPregunta), a);
	...
	/**Método que le translada al programa las preguntas que se encuentran en el fichero de preguntas medias
	 * 
	 */
	...
	/**Método que le translada al programa las preguntas que se encuentran en el fichero de preguntas dificiles
	 * 
	 */
	...
	/**Método para ver si la pregunta existe ya en el array de las preguntas de la partida
	 * @param j
	 * @param preguntasPartida
	 * @return
	 */
	private static boolean preguntaExistente(Pregunta j, Pregunta [] preguntasPartida) {
	...
	/**Método para ver las preguntas por pantalla, solo para saber si las preguntas se muestran correctamente
	 * 
	 */
	...
	public static void dinamicaJuego(){
		...
		for (int j = 0; j < euros.length; j++) {
			...
			System.out.println("...");
			System.out.println("Dinero por el que juegas en esta pregunta: "+ euros[j]+ "€");
			System.out.println("...");
			...
			if (resp=='S'){
				for(int i=0;i<comodines.length;i++){
					if(comodines[i].isUsado()==false)
						System.out.println((i+1)+".- "+comodines[i].getNombreComodin());
				}
